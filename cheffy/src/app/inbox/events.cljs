  ...
  (fn [{:keys [db]} [_ {:keys [message]}]]
    (let [uid               (get-in db [:auth :uid])
      ...
      {:db         (if existing-inbox-id
                     (update-in db [:inboxes existing-inbox-id :messages] conj m)
                     (assoc-in db [:inboxes new-inbox-id] {:messages     [m]
                                                           :participants #{uid cook}}))
       :dispatch-n [[:toggle-modal]
                    [:send-notification {:notify   cook
                                         :inbox-id (or existing-inbox-id new-inbox-id)}]]})))
